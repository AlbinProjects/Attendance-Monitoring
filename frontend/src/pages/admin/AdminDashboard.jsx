...
export default function AdminDashboard() {
  ...
  async function showDetails(metric) {
    ...
    try {
      ...
      setDetailsError(err?.response?.data?.detail || "Couldn't load the staff list.");
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-xs text-slate-muted">
        “Not yet checked in” reflects attendance staff with no attendance record for today — it isn’t a
  ...
  return (
    <Card className="!p-0 overflow-hidden">
      <div className="px-4 py-3 border-b border-border flex items-center justify-between gap-3">
        ...
        {!loading && details && (
          <span className="text-xs text-slate-muted">{details.rows.length} staff member{details.rows.length === 1 ? "" : "s"}</span>
        )}
      </div>

      {loading ? (
        <div className="px-4 py-8 text-center text-sm text-slate-muted">Loading staff details…</div>
      ) : error ? (
        <div className="px-4 py-8 text-center text-sm text-danger">{error}</div>
      ) : !details?.rows?.length ? (
        <div className="px-4 py-8 text-center text-sm text-slate-muted">No staff in this category.</div>
      ) : (
