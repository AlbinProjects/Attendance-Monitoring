  ...
  useEffect(() => {
    api.get("/admin/settings").then((res) =>
      setValues({
        ...
        advanced_desktop_monitoring_enabled: !!res.data.advanced_desktop_monitoring_enabled,
      })
    );
  }, []);
  ...
  async function handleSubmit(e) {
    ...
    try {
      const payload = {
        ...
        advanced_desktop_monitoring_enabled: values.advanced_desktop_monitoring_enabled,
      };
  ...
  return (
    <div className="space-y-5 max-w-xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <Card>
          <p className="text-xs uppercase tracking-wide text-slate-muted mb-3">Advanced desktop monitoring</p>
          <label className="flex items-start gap-2.5 text-sm">
            <input
              type="checkbox"
              checked={values.advanced_desktop_monitoring_enabled}
              disabled={!isSuperAdmin}
              onChange={(e) => update("advanced_desktop_monitoring_enabled", e.target.checked)}
              className="mt-0.5"
            />
            <span>
              <span className="font-medium text-ink">Enable advanced desktop monitoring</span>
              <p className="text-xs text-slate-muted mt-1">
                Off by default. When enabled, employees with monitored work sessions can use the desktop agent to provide privacy-safe activity signals from work applications.
              </p>
            </span>
          </label>
          <p className="text-xs text-slate-muted mt-3">
            The agent does not collect keystrokes, screenshots, code contents, URLs, browser history, terminal commands, clipboard contents, or document contents. Only Super Admin can change this setting.
          </p>
        </Card>
