...
function leaveLabel(type, period) { return type === "half_day" ? `Half-day${period ? ` (${period})` : ""}` : type ? type.charAt(0).toUpperCase() + type.slice(1) : type; }
...
export default function AdminCalendar() {
  ...
  return (
    <div className="space-y-5">
      ...
      <Card><div className="grid grid-cols-7 gap-1 text-center text-xs text-slate-muted mb-2">{["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map(x=><div key={x}>{x}</div>)}</div><div className="grid grid-cols-7 gap-1">{Array.from({length:leadingEmpty}).map((_,i)=><div key={`empty-${i}`} />)}{days.map(d=><button key={d.calendar_date} onClick={()=>setForm({calendar_date:d.calendar_date,day_type:d.day_type,is_working_day:d.is_working_day,name:d.name||""})} className={`min-h-16 rounded-lg border p-1.5 text-left ${d.day_type === "holiday" ? "bg-red-50 border-red-200" : d.calendar_date === `${year}-${String(month).padStart(2, "0")}-${String(new Date().getDate()).padStart(2, "0")}` && year === new Date().getFullYear() ? "bg-blue-50 border-blue-200 ring-1 ring-blue-300" : d.is_working_day ? "bg-white" : "bg-slate-50"}`}><div className="text-xs font-medium">{Number(d.calendar_date.slice(-2))}</div><div className="text-[10px] mt-1 truncate">{d.name||d.day_type.replaceAll("_"," ")}</div></button>)}</div></Card>
      ...
      <Card><h2 className="font-semibold mb-3">Leave records</h2><div className="space-y-2">{leaves.filter(x=>x.status!=="pending").map(x=><div key={x.id} className="flex flex-wrap items-center justify-between gap-2 border-b last:border-0 py-2 text-sm"><span>{employeeName(x.employee_id)} · {x.leave_date}</span><span className="capitalize">{leaveLabel(x.leave_type)} · {STATUS_LABEL[x.status] || x.status}{x.is_additional?" · additional":""}</span>{x.status==="approved"&&<button onClick={()=>cancel(x.id)} className="text-danger text-xs">Cancel</button>}</div>)}{leaves.filter(x=>x.status!=="pending").length===0&&<p className="text-sm text-slate-muted">No leave records.</p>}</div></Card>
    </div>
  );
}
