...
export default function Employees() {
  ...
  const canEdit = currentEmployee?.role === "admin" || isSuperAdmin;
  ...
  async function handleDelete(target) {
    ...
    const confirmed = window.confirm(
      `Permanently delete ${target.name} (${target.employee_code})?\n\nThis removes the login account and the employee's attendance, performance, and leave records. This cannot be undone.`
    );
    if (!confirmed) return;

    setSubmitting(true);
    try {
      await api.delete(`/admin/employees/${target.id}`);
      showToast(`${target.name} was permanently deleted.`);
      setModal(null);
      load();
    } catch (err) {
      showToast(err?.response?.data?.detail || "Couldn't delete the account.", "error");
    } finally {
      setSubmitting(false);
    }
  }
