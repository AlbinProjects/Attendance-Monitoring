...
export default function EmployeeDashboard() {
  ...
  const isApprovedFullDayLeave = todayLeave?.status === "approved" && todayLeave?.leave_type !== "half_day";
  ...
  const loadAll = useCallback(async () => {
    ...
    const results = await Promise.allSettled([
      ...
      api.get(`/calendar/leave/${(() => { const d = new Date(); return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`; })()}`),
    ]);
  ...
  return (
    <div className="space-y-4">
      ...
      {isApprovedFullDayLeave && (
        <Card className="!border-amber/30 !bg-amber-50/60">
          <p className="text-sm font-semibold text-amber">{`${String(todayLeave.leave_type || "Approved").replace("_", " ").replace(/\b\w/g, (c) => c.toUpperCase())} leave today`}</p>
          <p className="text-xs text-slate-muted mt-1">Attendance, breaks and performance update are not required today.</p>
          {todayLeave.reason && <p className="text-xs text-slate-muted mt-1">{todayLeave.reason}</p>}
        </Card>
      )}

      {!isApprovedFullDayLeave && missing.length > 0 && (
      ...
      {employee?.role === "employee" && !isApprovedFullDayLeave && !today?.check_in && !isOtherSite && !isOnDuty && <LaptopStatusCard connected={laptopConnected} />}
      ...
      {!isApprovedFullDayLeave && !((onDuty?.status === "approved" || onDuty?.status === "started") && !today?.check_in) && !isOtherSite && <PunchCard today={today} breakData={breakData} punching={punching} statusLabel={punchStatusLabel} onPunch={handlePunch} />}

      {employee?.role === "employee" && !isApprovedFullDayLeave && !isOtherSite && !isOnDuty && <BreakCard breakData={breakData} canStart={!!today?.check_in && !today?.check_out} onChanged={loadAll} />}

      {employee?.role === "employee" && !isApprovedFullDayLeave && advancedMonitoringEnabled && !isOtherSite && !isOnDuty && <ActivityCard activity={activity} desktopActivity={desktopAgentStatus} />}
      {employee?.role === "employee" && !isApprovedFullDayLeave && advancedMonitoringEnabled && isCheckedIn && !isOtherSite && !isOnDuty && (
