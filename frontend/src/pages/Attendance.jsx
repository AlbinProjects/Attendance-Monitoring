...
const DAY_LABELS = {
  ...
  leave: "Leave day",
  wfh: "Work From Home",
  other_site: "Work From Other Site",
};
...
function AttendanceDay({ day }) {
  ...
  return (
    <Card className={`${isToday ? "!border-blue-200 !bg-blue-50/50" : ""} !p-4`}>
      ...
      {remote?.work_mode === "other_site" && !leave ? (
        <div className="mt-3 rounded-lg bg-blue-50 border border-blue-100 px-3 py-2 text-sm">
          <p className="font-medium text-blue-700">Work From Other Site</p>
          <p className="text-xs text-slate-muted mt-1">{remote.site_name || "Other work site"}{remote.purpose ? ` · ${remote.purpose}` : ""}</p>
          <p className="text-xs text-blue-700 mt-1">No check-in/check-out and no 8-hour target are required for Other Site work.</p>
        </div>
      ) : leave ? (
      ...
      ) : remote?.work_mode === "wfh" && !leave ? (
        <>
          <div className="mt-3 rounded-lg bg-blue-50 border border-blue-100 px-3 py-2 text-sm">
            <p className="font-medium text-blue-700">Work From Home</p>
            <p className="text-xs text-slate-muted mt-1">Normal check-in/check-out and the 8-hour work target apply.</p>
          </div>
          {workApplicable && (
            <>
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3 text-sm">
            <Field label="Check-in" value={formatTime(attendance?.check_in) || "—"} mono />
            <Field label="Check-out" value={day.work_status === "checkout_missed" ? "Check-out missed" : (formatTime(attendance?.check_out) || "—")} mono alert={day.work_status === "checkout_missed"} />
            <Field label="Break time" value={formatDuration(day.total_break_seconds || 0)} mono />
            <Field label="Net work" value={day.net_work_seconds == null ? "—" : formatDuration(day.net_work_seconds)} mono />
          </div>
          <WorkTarget day={day} />
...
function WorkTarget({ day }) {
  if (day.work_mode === "other_site" || day.work_status === "not_required") return null;
