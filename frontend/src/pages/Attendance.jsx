...
export default function Attendance() {
  ...
  if (history.length === 0) {
    return (
      <Card className="text-center py-10">
        <p className="text-sm text-slate-muted">No attendance recorded yet.</p>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-lg font-semibold text-ink">Attendance history</h1>
        ...
        {history.map((row) => (
          <Card key={row.id} padded className="!p-4">
            ...
            <div className="grid grid-cols-2 gap-3 mt-3 text-sm">
              <div>
                <p className="text-slate-muted text-xs">Check-in</p>
                <p className="font-mono mt-0.5">{formatTime(row.check_in)}</p>
              </div>
              <div>
                <p className="text-slate-muted text-xs">Check-out</p>
                <p className="font-mono mt-0.5">{formatTime(row.check_out)}</p>
              </div>
            </div>
