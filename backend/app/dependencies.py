...
async def get_current_activity_agent(
    credentials: HTTPAuthorizationCredentials = Depends(bearer_scheme),
) -> Employee:
    """Authenticate the local desktop activity agent.

    Agent tokens are short-lived, scoped to one employee, and are never
    Supabase access tokens. The agent receives only this narrow token from
    the employee's already-authenticated browser session.
    """
    settings = get_settings()
    secret = settings.activity_agent_secret
    if not secret:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Desktop activity monitoring is not configured.")
    try:
        payload = jwt.decode(
            credentials.credentials,
            secret,
            algorithms=["HS256"],
            audience="activity-agent",
        )
    except JWTError as exc:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Desktop activity agent authorization expired. Please keep the attendance app open.",
        ) from exc

    if payload.get("typ") != "activity_agent" or not payload.get("sub"):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Invalid desktop activity agent authorization.",
        )
    ...
    employee = result.data if result else None
    if not employee or not employee.get("is_active") or employee.get("role") != "employee":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Desktop activity agent is not authorized for this account.",
        )
    return employee
