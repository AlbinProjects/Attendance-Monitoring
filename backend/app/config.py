...
class Settings(BaseSettings):
    ...
    # Legacy HS256 shared secret. Only used as a fallback for Supabase
    # projects that have NOT migrated to the newer asymmetric "JWT
    # Signing Keys" system — see app/services/jwks_service.py, which is
    # the primary verification path and needs no secret here at all
    # (it fetches the project's public key automatically). Safe to leave
    # blank on a migrated project.
    supabase_jwt_secret: str = ""
    # Secret used to sign short-lived desktop activity-agent tokens.
    # Falls back to the server-only Supabase service key when unset.
    activity_agent_secret: str = Field(default="", alias="ACTIVITY_AGENT_SECRET")
