...
def get_monthly_attendance(employee_id: str, year: int, month: int, settings: Settings) -> list:
    ...
    # Approved/assigned remote-work records are part of the attendance view.
    # WFH still follows normal attendance + 8-hour rules; Other Site is a
    # work-location record that intentionally has no check-in/out and no
    # 8-hour target.
    remote_rows = (client.table("remote_work_requests")
                   .select("id,employee_id,work_date,work_mode,site_name,purpose,status,planned_start,planned_end,approved_by,assignment_source")
                   .eq("employee_id", employee_id)
                   .gte("work_date", start.isoformat())
                   .lte("work_date", end.isoformat())
                   .in_("status", ["approved", "assigned", "started"])
                   .order("work_date", desc=False).execute().data or [])
    remote_by_date = {r["work_date"]: r for r in remote_rows}
    ...
    for day_num in range(first_day, monthrange(year, month)[1] + 1):
        ...
        remote = remote_by_date.get(iso)
        remote_mode = remote.get("work_mode") if remote else None
        net_work_seconds = None
        work_status = "not_applicable"
        if remote_mode == "other_site":
            # Other Site work is visible in attendance but has no normal
            # punch requirement and no 8-hour completion requirement.
            net_work_seconds = 0
            work_status = "not_required"
        elif calendar["is_working_day"] and not leave:
        ...
        if leave:
            day_status = "leave"
        elif remote_mode == "other_site":
            day_status = "other_site"
        elif remote_mode == "wfh":
            day_status = "wfh"
        ...
        out.append({
            ...
            "target_work_seconds": 0 if remote_mode == "other_site" else target_seconds,
...
def create_check_in(
    ...
    # An approved full-day leave takes precedence over normal attendance.
    # This is enforced server-side so the rule cannot be bypassed by calling
    # the check-in API directly. Half-day leave is intentionally different: it
    # may coexist with attendance and is handled by the half-day workflow.
    ...
    if approved_leave and approved_leave.get("status") == "approved" and approved_leave.get("leave_type") != "half_day":
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"You are on approved {leave_label} leave today. Check-in is not required.",
        )
...
def create_check_out(
    ...
    if approved_leave and approved_leave.get("status") == "approved" and approved_leave.get("leave_type") != "half_day":
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"You are on approved {leave_label} leave today. Check-out is not required.",
        )
