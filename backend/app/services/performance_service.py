...
def get_today_status(employee_id: str, settings: Settings) -> Dict[str, Any]:
    ...
    if (approved_leave and approved_leave.get("status") == "approved" and
            approved_leave.get("leave_type") != "half_day"):
        status_val = "not_available"
    elif not calendar_service.is_working_day(today):
        status_val = "not_available"
    else:
...
def submit_performance(
    ...
    approved_leave = calendar_service.get_leave_for_employee_date(employee_id, target_date)
    if (approved_leave and approved_leave.get("status") == "approved" and
            approved_leave.get("leave_type") != "half_day"):
        leave_label = str(approved_leave.get("leave_type") or "approved").replace("_", " ").title()
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"Performance is not required for approved {leave_label} leave.",
        )

    if not calendar_service.is_working_day(target_date):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Performance reports are not required on non-working days.",
        )
...
def get_missing_dates(
    ...
    """Only yesterday, and today after 5 PM, can trigger employee action."""
    now = get_office_now(settings)
    today = now.date()
    yesterday = today - timedelta(days=1)
    result = (get_service_client().table("performance_updates").select("work_date, submitted_at")
              .eq("employee_id", employee_id)
              .in_("work_date", [yesterday.isoformat(), today.isoformat()]).execute())
    submitted_dates = {r["work_date"] for r in (result.data or []) if r.get("submitted_at")}
    leave_rows = (get_service_client().table("employee_leaves").select("leave_date,leave_type,status")
                  .eq("employee_id", employee_id)
                  .in_("leave_date", [yesterday.isoformat(), today.isoformat()])
                  .execute().data or [])
    approved_full_day_leave_dates = {
        r["leave_date"] for r in leave_rows
        if r.get("status") == "approved" and r.get("leave_type") != "half_day"
    }
    missing = []
    if ((not joining_date or yesterday >= joining_date) and
        calendar_service.is_working_day(yesterday) and
        yesterday.isoformat() not in submitted_dates and
        yesterday.isoformat() not in approved_full_day_leave_dates):
        missing.append({"work_date": yesterday.isoformat(), "status": "missing"})
    if (is_performance_available(settings, at=now) and
        calendar_service.is_working_day(today) and
        today.isoformat() not in submitted_dates and
        today.isoformat() not in approved_full_day_leave_dates):
        missing.append({"work_date": today.isoformat(), "status": "missing"})
    return missing
...
def get_history(
    ...
    """Employee history from Sep 1, 2026; non-working dates are omitted."""
    today = get_office_today(settings)
    start = HISTORY_START_DATE
    if start > today:
        start = today
    result = (get_service_client().table("performance_updates").select("*")
              .eq("employee_id", employee_id).gte("work_date", start.isoformat())
              .lte("work_date", today.isoformat()).execute())
    ...
    while d >= start:
        ...
        if key in rows_by_date:
            ...
            leave = calendar_service.get_leave_for_employee_date(employee_id, d)
            approved_full_day_leave = leave and leave.get("status") == "approved" and leave.get("leave_type") != "half_day"
            if d == today:
                if calendar_service.is_working_day(d) and not approved_full_day_leave:
                    history.append({"work_date": key, "status": ("available" if is_performance_available(settings) else "not_available"), "submitted_at": None})
            elif calendar_service.is_working_day(d) and not approved_full_day_leave:
