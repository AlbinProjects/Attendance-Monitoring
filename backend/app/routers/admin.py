...
from app.schemas.salary import SalaryBaseResponse, SalaryBaseUpdateRequest, SalaryCalculateRequest, SalaryCalculationResponse
...
async def dashboard_details(
    ...
    settings = get_settings()
    try:
        return admin_service.get_dashboard_details(settings, metric)
...
async def get_salary_calculation(
    ...
    """Return the saved monthly salary calculation, if one exists."""
    return salary_service.get_saved(employee_id, year, month)


@router.get("/salary/base", response_model=SalaryBaseResponse)
async def get_salary_base(
    employee_id: str,
    _employee: dict = Depends(require_role("admin", "super_admin")),
):
    """Return the persistent default monthly salary for an Employee/Admin."""
    return salary_service.get_base(employee_id)


@router.put("/salary/base", response_model=SalaryBaseResponse)
async def update_salary_base(
    payload: SalaryBaseUpdateRequest,
    employee: dict = Depends(require_role("admin", "super_admin")),
):
    """Set/change the persistent default monthly salary for an Employee/Admin."""
    return salary_service.set_base(payload.employee_id, payload.salary, employee["id"])
...
async def calculate_salary(
    ...
    """Calculate and save one Employee/Admin monthly salary statement."""
    settings = get_settings()
    return salary_service.calculate(
