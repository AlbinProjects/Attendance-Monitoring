...
async def update_employee(
    ...
    if employee["role"] != "super_admin":
        forbidden = {k for k in ("role", "is_active") if k in update_data}
        if forbidden:
            from fastapi import HTTPException, status
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only Super Admin can change roles or account status.")
    ip_address = network_service.get_verified_client_ip(request)
