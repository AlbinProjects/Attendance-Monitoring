...
## Calendar and leave (Migration 008)

Apply `supabase/migrations/008_company_calendar_and_leaves.sql` once to databases that do not already contain the company calendar and employee leave tables. The employee UI reads the centralized calendar and leave balance; Admin/Super Admin can maintain company dates and grant/cancel leave. Non-working dates are excluded from employee performance requirements. Sundays are not automatically non-working: an Admin/Super Admin must explicitly mark a Sunday non-working when appropriate.

After deploying the backend and frontend, verify `/docs` contains `/api/calendar/*` and verify an explicitly configured non-working date is omitted from performance missing/history calculations.

## Desktop work-activity agent

The browser records activity in the attendance tab. For employees who do most
of their work in desktop applications (VS Code, terminals, Postman, database
clients, Office, design tools, etc.), install `desktop-agent/activity_agent.py`
on the work laptop.

The agent listens only on `127.0.0.1:17891`. The authenticated attendance
browser gives it a short-lived, employee-scoped token while the employee is
checked in. The agent sends only a generic activity heartbeat. It does not
collect keystrokes, screenshots, window titles, URLs, source code, document
contents, terminal commands, or clipboard contents.

### Render / Vercel

No database migration is required for the desktop agent. Deploy the backend
and frontend normally. The backend can optionally set `ACTIVITY_AGENT_SECRET`
to a long random secret. If omitted, the server-only Supabase service key is
used to sign the short-lived agent tokens.
